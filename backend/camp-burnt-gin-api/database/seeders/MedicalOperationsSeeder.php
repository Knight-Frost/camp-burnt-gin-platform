...
class MedicalOperationsSeeder extends Seeder
{
    ...
    private function treatmentLogsEthan(): void
    {
        ...
        if (TreatmentLog::where('camper_id', $c->id)->exists()) { return; }
    ...
    private function incidentsAndVisits(): void
    {
        ...
        if (MedicalIncident::where('camper_id', $ethan->id)->exists()) { return; }
    ...
    {
        if (MedicalFollowUp::where('title', 'Post-session Ethan transition protocol review')->exists()) { return; }
    ...
    {
        if (MedicalRestriction::exists()) { return; }
