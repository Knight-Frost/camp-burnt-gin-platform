 ...
 *  - Gradient overlay ensures the white card stays readable at all times.
...
interface FireflyDef {
  ...
  bottom:      number; // % from bottom edge
  size:        number; // diameter in px
  delay:       number; // rise animation-delay in s
  duration:    number; // rise animation-duration in s
  pulseDelay:  number; // glow-pulse animation-delay in s
}
  ...
  return (
    <div className="relative h-screen overflow-y-auto">
        ...
        <div
          ...
          style={{
            ...
            animation:          i === activeIdx
              ...
              : 'none',
            /* Cinematic grade: crush blacks, lift contrast, enrich colour */
      ...
      {/* ── Cinematic overlay stack ───────────────────────────────────────
          Layer 1 – radial vignette: darkens edges, draws eye to center
          Layer 2 – top bar: teal-tinted shadow (classic cinema shadow tint)
          Layer 3 – bottom bar: deep blue-black base for text legibility   */}
      ...
      />
      {/* Top cinematic shadow — warm dark tint */}
      <div
        style={{
          position:   'fixed',
          inset:      0,
      ...
      />
      {/* Bottom legibility gradient — cool dark base */}
      <div
        style={{
          position:   'fixed',
          inset:      0,
      ...
      {/* ── Film grain — SVG feTurbulence noise, mix-blend-mode: overlay ─── */}
      <div
        style={{
          position:        'fixed',
          inset:           0,
      ...
      {/* ── Fireflies — fixed so they stay visible while the page scrolls ─── */}
      {fireflies.map((ff) => (
        <div
          key={ff.id}
          style={{
